...
export default function Schedule() {
  ...
  const handleAddClass = async () => {
    ...
    // Validation
    if (!formData.subjectName || !formData.instructor || !formData.roomNumber || !formData.startTime || !formData.endTime || !formData.day) {
    ...
    const existingClass = classes.find(cls => 
      cls.subjectName && cls.subjectName.toUpperCase() === formData.subjectName.toUpperCase() && 
      ((formData.isLecture && cls.isLecture) || (formData.isLab && cls.isLab)) &&
      (!editingClass || cls._id !== editingClass._id)
    );

    if (existingClass) {
      console.log('Duplicate class error');
      setErrorMessage(`${formData.isLecture ? 'Lecture' : 'Laboratory'} class for ${formData.subjectName.toUpperCase()} already exists`);
      return;
    }
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (conflict) {
        setErrorMessage(`Time conflict with ${conflict.subjectName} at ${timeSlots[i]}`);
        return;
      }
    }
    ...
    try {
      if (editingClass) {
        ...
        const response = await fetch(`${API_URL}/schedule/${editingClass._id}`, {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          ...
          setClasses(classes.map(cls => cls._id === editingClass._id ? updatedClass : cls));
        } else {
        ...
        if (response.ok) {
          ...
          setErrorMessage('Failed to save class');
          return;
        }
      }
  ...
  const getClassForCell = (day, time) => {
    return classes.find(cls => cls.day === day && cls.timeSlots.includes(time));
  };
  ...
  const isLastSlot = (day, time) => {
    const classData = getClassForCell(day, time);
    return classData && classData.timeSlots[classData.timeSlots.length - 1] === time;
  };
                ...
                >
                  {showDetails ? (
                    <View style={styles.classInfo}>
                      <Text style={styles.subjectText}>{classData.subjectName}</Text>
                      <Text style={styles.instructorText}>{classData.instructor}</Text>
                      <Text style={styles.roomText}>{classData.roomPrefix} {classData.roomNumber}</Text>
                    </View>
                  ) : classData ? null : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.checkboxContainer}>
              <TouchableOpacity 
                style={[styles.checkbox, formData.isLecture && styles.checkboxSelected]}
                onPress={() => setFormData({...formData, isLecture: !formData.isLecture, isLab: false})}
              >
                <Text style={styles.checkboxText}>LEC</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.checkbox, formData.isLab && styles.checkboxSelected]}
                onPress={() => setFormData({...formData, isLab: !formData.isLab, isLecture: false})}
              >
            ...
            <View style={styles.dropdownContainer}>
              ...
              <ScrollView horizontal style={styles.daySelector}>
                {days.map((day) => (
                  <TouchableOpacity
                    key={day}
                    style={[styles.dayOption, formData.day === day && styles.daySelected]}
              ...
              <View style={styles.timeSelector}>
                ...
                <ScrollView style={styles.timeDropdown}>
                  {timeSlots.map((time) => (
                    <TouchableOpacity
                      key={time}
                      style={[styles.timeOption, formData.startTime === time && styles.timeSelected]}
              ...
              <View style={styles.timeSelector}>
                ...
                <ScrollView style={styles.timeDropdown}>
                  {timeSlots.map((time) => (
                    <TouchableOpacity
                      key={time}
                      style={[styles.timeOption, formData.endTime === time && styles.timeSelected]}
          ...
          <View style={styles.confirmModal}>
            ...
            {isEditing ? (
              <View style={styles.verticalButtons}>
                <TouchableOpacity 
                  style={[styles.verticalButton, {backgroundColor: '#007AFF'}]}
                  onPress={() => {
                    ...
                    setFormData({
                      ...
                      endTime: timeSlots[timeSlots.indexOf(selectedClass.timeSlots[selectedClass.timeSlots.length - 1]) + 1],
                ...
                <TouchableOpacity 
                  style={[styles.verticalButton, {backgroundColor: '#dc3545'}]}
                  onPress={handleDeleteClass}
                >
                  <Text style={styles.deleteButtonText}>Delete</Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={[styles.verticalButton, {backgroundColor: '#fff', borderWidth: 1, borderColor: '#ddd'}]}
              ...
              <View style={styles.confirmButtons}>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => {
                    // Add notes functionality can be added here
                    setShowCellModal(false);
                  }}
                >
